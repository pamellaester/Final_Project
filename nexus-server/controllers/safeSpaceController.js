...
const safeSpaceController = {
  createFormData: async (req, res) => {
    try {
      const { user_id, username, date, level,triggers, negative_thoughts } = req.body;

      await safePlaceModel.createFormData({ user_id, username, date, level,triggers, negative_thoughts });
